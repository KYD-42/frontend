...
function PlaceDetails() {
  ...
  return (
    <div className="touSoAver">
      ...
      <div>
        <section>
        <img src={place.logo} className="PlacesImg"  style={{marginTop: '400px', width: '1080px', height: '607.5px'}}  alt={place.name} />
          <h3>
            Type:{" "}
...
function PlaceDetails() {
  ...
  const [trigger, setTrigger] = useState(false);
  const [editingCommentId, setEditingCommentId] = useState(null);
  const [authenticatedUser, setAuthenticatedUser] = useState(null); // Assume authenticated user info is stored here
            ...
            {comments.length > 0 &&
              comments.map((comment) => (
                <div key={comment._id}>
                  <p>
                    <strong>{comment.userName}:</strong> {comment.text}
                    {authenticatedUser && authenticatedUser.userId === comment.userId && (
                      <>
